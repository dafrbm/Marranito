...
export default function App() {
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', maxWidth: 430, margin: '0 auto', position: 'relative', fontFamily: "'DM Sans', system-ui, sans-serif", color: C.text }}>
      ...
      {/* Bottom navigation */}
      <nav style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 430, background: C.surface, borderTop: `1px solid ${C.border}`, display: 'flex', zIndex: 50, paddingBottom: 'env(safe-area-inset-bottom)' }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{ flex: 1, padding: '10px 2px 12px', border: 'none', background: 'none', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, position: 'relative' }}>
            {tab === t.id && (
              <div style={{ position: 'absolute', top: 0, left: '20%', right: '20%', height: 2, background: C.accent, borderRadius: '0 0 2px 2px' }} />
            )}
            <span style={{ fontSize: 15, color: tab === t.id ? C.accent : C.hint, transition: 'color 0.2s' }}>{t.icon}</span>
            <span style={{ fontSize: 9, fontWeight: tab === t.id ? 700 : 400, color: tab === t.id ? C.accent : C.hint, letterSpacing: '0.06em', transition: 'color 0.2s' }}>
              {t.label.toUpperCase()}
            </span>
          </button>
        ))}
